...
async def scan_host_ports(ip, ports, sudo_passwd):
...
async def high_port_check(ip, random_ports, sudo_passwd, rosport="11311"):
    # Scan ros port separately to avoid blocking due to scanning to other ports
    _, ports1 = await scan_host_ports(ip, rosport, sudo_passwd)
    _, ports2 = await scan_host_ports(ip, random_ports, sudo_passwd)
    ...
    for target in targets:
        if rosport:
            tasks.append(asyncio.ensure_future(
                high_port_check(target, ports, sudo_passwd)))
        else:
            tasks.append(asyncio.ensure_future(
                scan_host_ports(target, ports, sudo_passwd)))
